...
test "tokenizer general test" {
    ...
    const rom = try assembler.assemble();
    ...
    const nrows = try std.math.divCeil(usize, rom.len, 10);
    for (0..nrows) |row| {
        for (0..10) |col| {
            const i = row * 10 + col;
            if (i >= rom.len) break;
    ...
    try std.testing.expectEqual(uxn.LIT2, rom[0x00]);
    try std.testing.expectEqualSlices(u8, &[2]u8{0x01, 0x13}, rom[1..2+1]);
